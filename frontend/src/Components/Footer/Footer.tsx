...
const Footer = (props: Props) => {
  ...
  const handleJoin = async () => {
    if (email) {
      const emailRef = ref(database, 'subscribers/' + email.replace(/\./g, ','));
    ...
    <div className="select-none py-24 px-4 md:px-8 lg:px-16 xl:32 2xl:px-64 bg-gray-100 text-sm mt-8">
      ...
      <div className="flex flex-col md:flex-row justify-between gap-24 p-8">
        ...
        <div className="w-1/2 flex flex-col gap-2">
          ...
          <p className="font-bold text-xl">South Africa</p>
        ...
        <div className=" w-full md:w-1/2 lg:w-1/4 flex flex-col justify-between xl:w-full gap-2 p-4 border-red-300">
          ...
          <div className="flex">
            <input
              ...
              className="p-4 w-3/4"
            />
            <button onClick={handleJoin} className="w-1/4 bg-gray-700 text-white">JOIN</button>
          </div>
          {successMessage && (
            <p className="mt-2 text-green-600">{successMessage}</p> // Success message
          )}
